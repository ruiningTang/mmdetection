...
class GABottleneck_b3(_Bottleneck):
    expansion = 4
    ...
    def __init__(self,
        ...
        """
        super(GABottleneck_b3, self).__init__(inplanes, planes, **kwargs)
        ...
        if not self.with_dcn or fallback_on_stride:
            ...
            self.bns_3 = nn.ModuleList(bns_3)
    ...
    def forward(self, x):
        ...
        def _inner_forward(x):
            ...
            spx3 = torch.split(x3, self.width, 1)
            sp3 = self.convs_3[0](spx3[0].contiguous())
            sp3 = self.relu(self.bns_3[0](sp3))
            out3 = sp3
            for i in range(1, self.path - 1):
                if self.stage_type == 'stage':
                    sp3 = spx3[i]
                    ...
                    sp3 = sp3 + spx3[i]
                sp3 = self.convs_3[i](sp3.contiguous())
                sp3 = self.relu(self.bns_3[i](sp3))
                out3 = torch.cat((out3, sp3), 1)

            if self.stage_type == 'normal' or self.conv2_stride == 1:
                out3 = torch.cat((out3, spx3[-1]), 1)
            elif self.stage_type == 'stage':
                out3 = torch.cat((out3, self.pool(spx3[-1])), 1)
            ...
            out3 = out2 + out3
            out3 = self.convs_2[1](out3)
            out3 = self.relu(self.bns_2[1](out3))
            #print('output y:', out1.shape, out2.shape,out3.shape)
...
class GALayer_b3(nn.Sequential):
        ...
        for i in range(1, num_blocks):
            layers.append(
                block(
                    ...
                    branch=branch,
                    **kwargs))
        super(GALayer_b3, self).__init__(*layers)


@BACKBONES.register_module()
class GANet_b3(ResNet):
    """Res2Net backbone.
    Args:
        scales (int): Scales used in Res2Net. Default: 4
        base_width (int): Basic width of each scale. Default: 26
    ...
    def init_weights(self, pretrained=None):
        ...
        if isinstance(pretrained, str):
            ...
            if self.dcn is not None:
                for m in self.modules():
                    if isinstance(m, GABottleneck_b3):
